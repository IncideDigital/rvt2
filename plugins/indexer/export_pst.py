...
class CreatePstHtml(base.job.BaseModule):
    """
    ...
    def read_config(self):
        super().read_config()
        self.set_default_config('outfile', 'index.html')
    ...
    def _export_item(self, item):
        ...
        Parameters:
            item (dict): item to export (Message, Appointment, Activity, Contact, Meeting, Note or Task)
        """

        base_path = os.path.join(self.casedir, os.path.dirname(item['path']))
        output_filename = os.path.join(base_path, self.myconfig('outfile'))
        # if base.utils.check_file(output_filename):
        #     return

        tipe = item['content_type'][4:]
        ...
        if tipe == "Message":
            fich = os.path.join(base_path, "OutlookHeaders.txt")
        ...
        if os.path.isfile(os.path.join(base_path, "InternetHeaders.txt")):
            int_headers = repl_lt_gt(get_text(os.path.join(base_path, "InternetHeaders.txt")))
        if os.path.isfile(os.path.join(base_path, "ConversationIndex.txt")):
            conv_ind = get_text(os.path.join(base_path, "ConversationIndex.txt"))
        if os.path.isfile(os.path.join(base_path, "Recipients.txt")):
            recipientes = get_text(os.path.join(base_path, "Recipients.txt"))
        if os.path.isfile(os.path.join(base_path, "RestOutlookHeaders.txt")):
            R_out_head = ["RestOutlookHeaders.txt", get_text(os.path.join(base_path, "RestOutlookHeaders.txt"))]
        extra_style = ""
        body = ""
        for fich in os.listdir(base_path):
            if fich.startswith("Message"):
                fichero = os.path.join(base_path, fich)
                if fich.endswith(".html"):
                    ...
                    body = self.tika_parser.run(os.path.join(base_path, fich))[0]['content'].replace("\n", "<br>\n")
                ...
                if os.path.isfile(fichero):
                    ...
                    for f2 in os.listdir(os.path.join(att_dir, f)):
                        if srch_type.search(f2):
                            ...
                            attach.append([os.path.join(r, f2, "index.html"), total_bytes])

        temporal = dict()
        temporal["Client submit time"] = item.get("email_submit_time", "")
        temporal["Creation time"] = item.get("creation_time", "")
        temporal["Delivery time"] = item.get("email_delivery_time", "")
        temporal["Flags"] = item.get("email_flags", "")
        temporal["Importance"] = item.get("importance", "")
        temporal["Modification time"] = item.get("modification_time", "")
        temporal["Priority"] = item.get("priority", "")
        temporal["Sender name"] = item.get("email_from", "")
        if "email_conversation_topic" in item.keys() and item['email_conversation_topic']:
            temporal["Conversation topic"] = repl_lt_gt(item.get("email_conversation_topic"))
        subject = item.get("email_subject", "")
        if subject:
            subject = repl_lt_gt(subject)
        ...
        source = ""
        if item['path'].find('.export') > -1:
            source = os.path.dirname(item['path']).split(".export/")[1]
        elif item['path'].find('orphan') > -1:
            source = os.path.dirname(item['path']).split(".orphan/")[1]
        elif item['path'].find('recovered') > -1:
            source = os.path.dirname(item['path']).split(".recovered/")[1]
